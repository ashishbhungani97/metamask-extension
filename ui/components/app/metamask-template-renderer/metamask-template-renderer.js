...
function getElement(section) {
  ...
  if (!Element) {
    throw new Error(
      `${element} is not in the safe component list for BlockStar template renderer`,
    );
  }
  return Element;
}
    ...
    <>
      {sections.reduce((allChildren, child) => {
        ...
        if (typeof child === 'string') {
          ...
          if (!child.key) {
            throw new Error(
              'When using array syntax in BlockStar Template Language, you must specify a key for each child of the array',
            );
          }
